...
public class LightBulbSwitcher {
  public int getShiningCounts(int[] arr) {
    ...
    for (int i = 1; i <= arr.length; i++) {
      sum += arr[i - 1]; // turn on Blub: arr[i-1]
  ...
  public static void main(String[] args) {
    LightBulbSwitcher engine = new LightBulbSwitcher();
    System.out.println(engine.getShiningCounts(new int[]{2, 1, 3, 5, 4}));
    System.out.println(engine.getShiningCounts(new int[]{2, 3, 4, 1, 5}));
    System.out.println(engine.getShiningCounts(new int[]{5, 4, 3, 2, 1}));
  }
}
